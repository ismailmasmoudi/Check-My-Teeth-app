...
export class ToothSelectorComponent {
  ...
  toothIdentifier: string = ''; // Stores the currently selected tooth (e.g., "18")
  ...
  teeth = {
    ...
    lowerLeft: [48, 47, 46, 45, 44, 43, 42, 41],  // From wisdom tooth to central incisor
    lowerRight: [31, 32, 33, 34, 35, 36, 37, 38]   // From central incisor to wisdom tooth
  };

  // Method to select a tooth from the schema
  selectTooth(toothId: number): void {
    this.toothIdentifier = toothId.toString();
  }

  confirmSelection(): void {
    if (this.toothIdentifier.trim()) {
      this.toothSelected.emit(this.toothIdentifier.trim());
    }
  }
}